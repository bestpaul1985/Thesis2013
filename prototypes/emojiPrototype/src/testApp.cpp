...
void testApp::draw(){
    ...
    for (int i = 0; i<5; i++) {
        ...
        //numbercount
        ofDrawBitmapString(ofToString(emotionOccurCount[i]), -(r+10)*sin(angle),-(r+10)*cos(angle));
        //data input
        
        int currentValue = ofMap(emotionOccurCount[i], 0, mostOccur, 10, 90);
        
        data[i].set(-currentValue*sin(angle), -currentValue*cos(angle));
        ofPopMatrix();
    }
    ...
    ofBeginShape();
    for (int i=0; i<5; i++) ofVertex(data[i]);
    
    ofEndShape();
    ofSetLineWidth(3);
    ofSetColor(ofColor::lightBlue);
    for (int i=0; i<5; i++) ofLine(0, 0, data[i].x, data[i].y);
