...
void ttChar::setup(ofxBox2d &characterWorld,
    ...
    b2FixtureDef fixture = character.fixture;
    fixture.filter.categoryBits = 1;
    cout<<character.fixture.shape->GetType() <<endl;
...
//----------------------------------------------
void ttChar::contact(ttRopeJoint &_Rope){
    ttRopeJoint *rope = &_Rope;
    ofPoint pos = rope->getLastPos;
 
    ttSetData* sd = (ttSetData*)character.getData();
    if (charNum == 0) {
        if (sd->bHooked&& character.getPosition().y < 512)
        {
            character.setPosition(pos.x,pos.y-character.getHeight());
            rope->bHooking = true;
