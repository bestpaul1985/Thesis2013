...
//    joints.push_back(joint);
    ...
    if (joints.empty()) {
        ofxBox2dRect rect;
        rect.setPhysics(40.0f, 0.0f, 0.2f);
        rect.setup(box2d.world, start.getPosition().x, start.getPosition().y, 15, 15);
        ...
        revoluteJointDef.localAnchorA.Set(p.x, p.y);
        p = screenPtToWorldPt(ofPoint(0,15));
        revoluteJointDef.localAnchorB.Set(p.x, -p.y);
        revoluteJointDef.enableLimit = true;
        revoluteJointDef.lowerAngle = -PI/8;
        revoluteJointDef.upperAngle = PI/8;
        ...
        ofxBox2dRect rect;
        rect.setPhysics(20.0f, 0.0f, 0.2f);
        rect.setup(box2d.world, start.getPosition().x, start.getPosition().y, 2, 10);
        rect.body->GetFixtureList()->SetSensor(true);
        rects.insert(rects.begin(), rect);
